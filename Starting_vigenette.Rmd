...
```
You can plot the following DAG with the Rgraphviz library if desired
...
Setting effect sizes requires writing generalized linear models (GLMs) within the distribution. Below is example code that discloses how to write them. Our RShiny app can write the GLM code for you as long as all nodes distributions are Bernoulli or Gaussian. 
...
default_run = varied_runs(500, my_dag, "A","C", n = 10000, family = quasibinomial(link = "logit")) #default
probit_run = varied_runs(500, my_dag,  "A","C",  n = 10000, family = quasibinomial(link = "probit"))
cloglog_run = varied_runs(500, my_dag,  "A","C", n = 10000, family = quasibinomial(link = "cloglog"))
...
matching_run1 = varied_runs(500, my_dag, "B", "C", "A",  n = 10000, match_methods = c("Mahalanobis"), ratio =1)
...
Users can control the number of controls that are matched to cases with the ratio argument.  Matching methods do best with a binary exposure that has low prevalence. In this example, since exposure **B** was dichotimized for p=0.25, a ratio of 3 starts biasing results due to the reuse of controls being matched to the cases.
```{r, eval=FALSE}


matching_run2 = varied_runs(500, my_dag, "B", "C", "A",  n = 10000, match_methods = c("Mahalanobis"), ratio =2)
matching_run3 = varied_runs(500, my_dag, "B", "C", "A",  n = 10000, match_methods = c("Mahalanobis"), ratio =3)

tot_match = reparse_runs(list(matching_run1,matching_run2,matching_run3), "Mahalanobis", 
                         c("1 case to 1 control", "1 case to 2 controls", "1 case to 3 controls"))
head(tot_match$sample_population,3)
ci_ridges(tot_match)
beta_summary(tot_match)
```
```{r,echo=FALSE, message=FALSE}
tot_match = reparse_runs(list(matching_run1,matching_run2,matching_run3), "Mahalanobis", 
                         c("1 case to 1 control", "1 case to 2 controls", "1 case to 3 controls"))
ci_ridges(tot_match)
combined_summary = round(beta_summary(tot_match), 4)
knitr::kable(combined_summary)
```
