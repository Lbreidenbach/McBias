...
default_run = varied_runs(100, my_dag, "B", "C", "A", n = 10000) #default
probit_run = varied_runs(100, my_dag, "B", "C", "A",  n = 10000, family = quasibinomial(link = "probit"))
cloglog_run = varied_runs(100, my_dag, "B", "C", "A",  n = 10000, family = quasibinomial(link = "cloglog"))
...
knitr::kable(combined_summary)
```
McBias will default to a logistic regression for a binary outcome and a linear regression for a continuous outcome. Since we set our GLM for outcome node C with log odds ratio coefficients and the logit linker function, the default regression converges on the effect size estimate the best. Depending on the distributions you set for the outcome, a different regression type may converge better (ex. family = "poisson" for an outcome with a poisson distribution).
