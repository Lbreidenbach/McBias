...
Now that a DAG object is set, simulations can began. create_data() allows a single dataset to be generated. Binary nodes are in the integer class by default. The user may reclassify them if desired.

```{r, results=FALSE}
sim_df = create_data(my_dag, 10000)
factor_sim_df = create_data(my_dag, 10000, reclassify = as.factor)
```
```{r}
str(sim_df)
str(factor_sim_df)
...
Now that we have a working DAG object (my_dag) we can start to analyze it in various ways.  varied_runs() generates multiple datasets and analyzes them as specified. It returns the following summary statistics for each dataset:
...
* **If the outcome is Bernoulli**:
    + "logistic_regression* is run. "ps_weighting" or propensity score weighting is also run if there are covariates.
* **If the outcome is Gaussian**:
    + "linear_regression" is run. "ps_weighting" or propensity score weighting is also run if there are covariates.
We'll fill in the arguements below:

* **runs** = 500
    + 500 datsets will be created and analyzed
* **dag** = my_dag
...
```{r}
knitr::kable(head(ideal$calculated_ate))

#We can further splice to grab a specific method
head(ideal$calculated_ate[,"logistic_regression"])

ideal_sum_df = data.frame("logistice regression effect estimate" = round(mean(ideal$calculated_ate[,"logistic_regression"]), 3),
                          "true effect estimate" = mean(ideal$set_ate[,"logistic_regression"]),
                          "average p value" = round(mean(ideal$p_values[,"logistic_regression"]), 3))

knitr::kable(ideal_sum_df)
```
```{r, echo=FALSE}
ate = round(mean(ideal$calculated_ate[,"logistic_regression"]), 3)
tb = mean(ideal$set_ate[,"logistic_regression"])
pval = round(mean(ideal$p_values[,"logistic_regression"]), 3)
```
So here we can see that when node **A** is adjusted on and logistic regression is used, we get a mean effect size estimate of `r ate` between **B** and **C** over the 500 simulated datasets. Since **B** has no effect size on **C**, the true effect size is `r tb`. The average p_value is `r pval`. This is as expected. The estimate is close to 0 which corresponds to a p value near 0.5.
...
**ideal** was the proper analysis. The collider **D** was not adjusted, and the confounder **A** was. We can see in this case that the effect size estimate does not drift from the mean. The summary of ideal's simulation reflects this. It has very little bias. Since the true effect size was 0 in this case, we see that the rejection rate approximately equals the alpha value, which we set at 0.05. Coverage also approximately equals 95%. Overall this means that when everything is adjusted properly one gets results with very little bias, and the true effect size approximately equals the calculated effect size.

What if the ideal couldn't be obtained? Suppose we didn't know to adjust for **A** or were given a data set that was stratified on **D**? We can simulate the following scenarios to estimate how much bias these two scenarios would cause.
...
selection_bias = varied_runs(500, my_dag, "B", "C", sb = "D", n = 400000) #n is larger here. D has a prevalence of 0.25 and only samples with D will make it to the cut data set that is analyzed. If we want n to be similar to the runs we are comparing it too, we'll have to multiply n * 1/p 
...
For this example, I want to compare the "logistic regression" method results of each scenario, so method = "logistic_regression".
```{r}
