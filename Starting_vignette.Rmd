...
Setting effect sizes requires writing generalized linear models (GLMs) within the distribution within `setNode()`. First we can look up information about available distributions by referencing the `jagsDists` dataframe.
```{r, eval=FALSE}
...
my_dag <- setNode(network = my_dag, 
                  node = C, 
                  nodeType = "dbern", 
                  prob = paste0("ilogit(",0.8," * A + ",set_p(0.9, 0.8*0.5),")")
                  )
...
my_dag <- setNode(network = my_dag,
                 node = D,
                 nodeType = "dbern", 
                 prob = paste0("ilogit(",0.05," * B + ", 1.2," * C + ",set_p(0.25, 1.2 * 0.9 + 0.05 * 0 ),")")
                 )
```
...
```{r, results=FALSE}
sim_df <- create_data(my_dag, 10000, seed=137)
head(sim_df, 5)
...
ideal <- varied_runs(runs = 500,
                    ...
                    covariates = "A", 
                    n = 100000,
                    seed = 46 #for seed to properly pass to JAGS it must be set within varied runs
                    )
```
...
selection_bias <- varied_runs(runs = 500,
                             ...
                             sb = "D",
                             n = 400000,
                             seed = 46) #n is larger here. D has a prevalence of 0.25 and only samples with D will make it to the cut dataset that is analyzed. If we want n to be similar to the runs we are comparing it too, we'll have to multiply n * 1/p 
